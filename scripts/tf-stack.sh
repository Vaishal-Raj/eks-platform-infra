...
#
# Needs build/<client>/<env>/ first:  python scripts/config.py resolve --client <client> --env <env>
set -euo pipefail
...
# -reconfigure: one stack folder serves every client, so always re-point the backend
